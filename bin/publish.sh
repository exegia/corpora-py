...
#
# publish.sh
# Shell equivalent of scripts/publish.py
#
# Publish a new release via GitHub Actions.
#
# Modes:
#   tag (default)
#     Bump version, commit, tag, push. The publish workflow triggers on tag.
#
#   --dispatch
...
#   ./scripts/publish.sh --dispatch
#   ./scripts/publish.sh --dispatch --publish
#   ./scripts/publish.sh --dispatch --ref some-branch
#
...
# As written in the original Python — may need adjustment if layout changed
...
set_version() {
  ...
  for path in "$PYPROJECT" "$VERSION_FILE"; do
    if [[ ! -f "$path" ]]; then
    ...
    text=$(cat "$path")
    local updated
    # Replace only the first occurrence of "old" with "new" (quoted)
    updated=$(python3 -c '
import sys, re
...
' "$old" "$new" <<< "$text")
    ...
    if [[ "$updated" == "$text" ]]; then
      echo "error: version string not found in $path" >&2
      exit 1
    fi
...
commit_and_tag() {
  local version="$1"
  local tag="v${version}"
  run git add "$PYPROJECT" "$VERSION_FILE"
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -h|--help)
        cat <<'EOM'
Publish helper (shell port of scripts/publish.py)

  ./scripts/publish.sh                 # bump patch, commit, tag, push
  ./scripts/publish.sh patch
  ./scripts/publish.sh minor
  ./scripts/publish.sh major
  ./scripts/publish.sh 1.2.3
