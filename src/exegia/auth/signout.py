...
The shared client in :mod:`exegia.supabase.client` uses the service-role key
...
(:func:`exegia.supabase.authentication.admin_sign_out`).
...
from exegia.supabase import authentication
from exegia.supabase.authentication import SignOutScope
from exegia.utils.constant import PROJECT_ROOT
    ...
    *,
    ...
    if save_activity:
        try:
            ...
            logger.warning("Failed to persist session activity for %s: %s", user_id, exc)
