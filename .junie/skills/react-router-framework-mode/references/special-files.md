...
**Note:** Be defensive in your `Layout` component since it renders the `ErrorBoundary`. Use `useRouteLoaderData("root")`
instead of `useLoaderData` to handle cases where the loader threw. See [error-handling.md](./error-handling.md) for
...
The server entry point for rendering HTML responses. React Router provides a default for Node, but you may need to
customize for other runtimes (Cloudflare, Deno).
...
**Note:** Values exported from `.client` modules are `undefined` on the server. Only use them in `useEffect` or event
handlers.
...
**Don't use flat routes when nesting makes sense** - see [routing.md](./routing.md#layout-routes-use-them) for proper
