...
Actions handle data mutations (create, update, delete). After an action completes, all loaders on the page automatically
revalidate.
...
**Use `useFetcher` as the default for mutations** - it provides independent state, no page navigation, and enables
optimistic UI.
...
See [pending-ui.md](./pending-ui.md#optimistic-ui-with-usefetcher-recommended-pattern) for complete patterns.
