...
The shared client in :mod:`exegia.supabase.client` is built with the
...
from exegia.supabase.authentication import (
    OAuthProvider,
    get_current_user,
    link_identity,
)
...
def link_identity_to_current_user(
    ...
    *,
    ...
    if current.is_anonymous:
        ...
        logger.warning(
            "User %s already has a linked %s identity", current.id, provider
        )
    else:
