...
This is a Python library (`exegia`, published as `corpora-py`) — no web server, just an MCP server entry point and importable modules. Everything lives in `src/exegia/`.

### Module layers

**`exegia.mcp`** — The only user-facing surface. A FastMCP server exposing 11 tools to AI clients (Claude Desktop, etc.). The `cf-mcp` CLI entry point lives here. `corpus.py` holds the singleton `CorpusManager` that loads/manages `context-fabric` (`cfabric.Fabric`) corpora at runtime.

**`exegia.supabase`** — Low-level layer. `client.py` creates a single synchronous Supabase client using the service-role key (bypasses RLS; no per-user session stored). `authentication.py` provides thin, typed wrappers around `supabase.auth.*` methods.

**`exegia.auth`** — High-level application layer on top of `exegia.supabase`. After any sign-in that produces a session, it checks whether a matching `public.users` record exists and returns a `SignInResult` or `CurrentUser` dataclass. All per-user context is passed explicitly via JWT/token args — the Supabase client is stateless with respect to sessions.

**`exegia.corpus`** — Git-based dataset fetcher. Shallow-clones a repo and locates Text-Fabric dataset directories (those containing both `otext.tf` and `otype.tf`).

**`exegia.utils`** — EPUB/HTML → Text-Fabric converters. These produce a node hierarchy (`book → chapter → element → paragraph → word`) that the MCP server can load and query.

**`exegia.models` / `exegia.schemas`** — Enums, data models, and Pydantic schemas shared across modules.
...
2. `CorpusManager.load(path)` wraps `cfabric.Fabric` and holds `(Fabric, api)` pairs keyed by name.
