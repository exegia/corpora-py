...
def search_continue(cursor_id: str, limit: int = 20) -> str:
    ...
    footer = (
        f"\n{remaining} more. cursor_id: {cursor_id}"
        if remaining > 0
        else "\nEnd of results."
    )
    return "\n".join(lines) + footer
...
def _parse_section_ref(ref: str, api: Any) -> tuple[Any, ...] | None:
    ...
    if ":" in ref:
        ...
        if n == 3:
            ...
            try:
                return (
                    (book, int(chapter), int(verse)) if chapter else (book, int(verse))
                )
            except ValueError:
                return None
    else:
...
def get_node_features(
    ...
    col_w = 10
    header = f"{'node':>8}  {'section':<20}" + "".join(
        f"  {n:<{col_w}}" for n in features
    )
    sep = "-" * len(header)
    rows = [header, sep]
    ...
    for node in nodes:
        section = _section_ref(node, api)
        vals = "".join(
            f"  {str(f.v(node) if f else ''):<{col_w}}" for _, f in feat_objs
        )
