-- Keep private working-draft metadata out of legacy broad document policies.
ALTER TABLE public.corpus_documents ADD COLUMN ai_private boolean NOT NULL DEFAULT false;
UPDATE public.corpus_documents SET ai_private=true
    WHERE id IN (SELECT document_id FROM public.corpus_ai_drafts);
ALTER TABLE public.corpus_documents ENABLE ROW LEVEL SECURITY;
CREATE POLICY corpus_documents_ai_private ON public.corpus_documents AS RESTRICTIVE
    FOR ALL TO PUBLIC USING (NOT ai_private) WITH CHECK (NOT ai_private);
REVOKE TRUNCATE ON public.corpus_documents FROM anon,authenticated;
GRANT SELECT,INSERT,UPDATE ON public.corpus_documents TO service_role;
...
BEGIN
    ...
    IF p_action = 'capabilities' THEN
        RETURN jsonb_build_object('mutation_api',1,'draft_api',1);
    ELSIF p_action = 'provision' THEN
        -- Only the authenticated application server calls this, after checking
        -- exact source-job ownership and verifying the staged immutable bytes.
        -- One document per owner/job; an existing binding always wins a retry.
        operation_id := (p_payload->>'document_id')::uuid;
        PERFORM pg_advisory_xact_lock(hashtextextended(operation_id::text,0));
        SELECT * INTO d FROM public.corpus_ai_drafts WHERE document_id=operation_id;
        IF FOUND THEN
            IF d.owner<>p_owner OR d.corpus<>p_payload->>'corpus' OR NOT EXISTS (
                SELECT 1 FROM public.corpus_documents WHERE id=d.document_id
                    AND job_id=p_payload->>'job_id' AND ai_private)
                THEN RAISE EXCEPTION 'AI_CONFLICT'; END IF;
            RETURN to_jsonb(d);
        END IF;
        IF p_payload->>'corpus' IS DISTINCT FROM 'draft:'||operation_id::text
            OR p_payload->>'bucket' IS DISTINCT FROM 'corpus-ai-drafts'
            OR p_payload->>'job_id' IS NULL OR p_payload->>'name' IS NULL
            THEN RAISE EXCEPTION 'AI_INVALID'; END IF;
        IF EXISTS (SELECT 1 FROM public.corpus_documents WHERE id=operation_id)
            THEN RAISE EXCEPTION 'AI_CONFLICT'; END IF;
        INSERT INTO public.corpus_documents(id,name,source,path,filename,job_id,status,ai_private)
            VALUES(operation_id,p_payload->>'name','upload',p_payload->>'object_key',
                operation_id::text||'.corpus',p_payload->>'job_id','converted',true);
        INSERT INTO public.corpus_ai_drafts(document_id,owner,corpus,bucket,object_key,revision,digest,version)
            VALUES(operation_id,p_owner,p_payload->>'corpus',p_payload->>'bucket',p_payload->>'object_key',
                (p_payload->>'revision')::uuid,p_payload->>'digest',p_payload->>'version') RETURNING * INTO d;
        RETURN to_jsonb(d);
    ELSIF p_action = 'drafts' THEN
        IF (p_payload->>'limit')::int NOT BETWEEN 1 AND 101 OR (p_payload->>'offset')::int < 0
            THEN RAISE EXCEPTION 'AI_INVALID'; END IF;
        SELECT coalesce(jsonb_agg(to_jsonb(q) ORDER BY q.document_id),'[]') INTO result
            FROM (SELECT * FROM public.corpus_ai_drafts WHERE owner=p_owner AND corpus='draft:'||document_id::text
                ORDER BY document_id LIMIT (p_payload->>'limit')::int OFFSET (p_payload->>'offset')::int) q;
        RETURN result;
    ELSIF p_action = 'register' THEN
        ...
        UPDATE public.corpus_documents SET ai_private=true WHERE id=d.document_id;
        RETURN to_jsonb(d);
