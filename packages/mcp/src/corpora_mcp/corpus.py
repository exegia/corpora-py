...
class CorpusManager:
    ...
    def load(
        self, path: str, name: str | None = None, features: list[str] | None = None
    ) -> str:
        try:
            from cfabric import Fabric  # type: ignore[import]
        except ImportError as exc:
            raise ImportError(
                "context-fabric is required: pip install context-fabric"
            ) from exc
        ...
        api = (
            tf.load(" ".join(features), silent="deep")
            if features
            else tf.loadAll(silent="deep")
        )
    ...
    def get_api(self, name: str | None = None) -> Any:
        ...
        if target is None:
            raise RuntimeError(
                "No corpus loaded. Pass --corpus when starting the server."
            )
