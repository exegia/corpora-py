    ...
    <div className="flex w-full flex-col gap-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Provide details and select the dataset archive.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Form method="post" encType="multipart/form-data" className="space-y-4">
            <div className="space-y-1">
              ...
              <Input id="name" name="name" value={name} onChange={(e) => setName(e.target.value)}
                     placeholder="e.g. BHSA" required />
              <p className="text-xs text-neutral-500">A short, unique identifier for the corpus.</p>
            </div>

            <div className="space-y-1">
              <label htmlFor="description" className="block text-sm font-medium">Description</label>
              <Textarea id="description" name="description" value={description}
                        onChange={(e) => setDescription(e.target.value)} placeholder="What does this corpus contain?"
                        rows={3} />
            </div>

            <div className="space-y-1">
              <label htmlFor="archive" className="block text-sm font-medium">Dataset archive</label>
              <input id="archive" name="archive" type="file" accept=".zip,.tar,.gz,.tgz"
                     ...
                     required />
              <p className="text-xs text-neutral-500">ZIP or tarball containing the Text-Fabric dataset.</p>
            </div>
