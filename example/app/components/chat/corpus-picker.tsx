...
/**
 * The chat route's required first step: pick which published `.corpus`
 * archive to load. Lists the archives in the Hugging Face Hub storage repo
 * (`GET /storage` — same source as Explore) and reports the chosen filename
 * via `onSelect`.
 */
...
const fetchStoredCorpora = async (): Promise<ListState> => {
  try {
      ...
      {state.status === "ready" && corpora.length === 0 && (
        <Empty className="border-2 border-dashed">
          <EmptyHeader>
            ...
            <EmptyDescription>
              Convert a document and use “Publish to Hugging Face” to make it
              chattable here.
            </EmptyDescription>
          </EmptyHeader>
          <Button
          ...
          <Empty className="border-2 border-dashed">
            <EmptyHeader>
              ...
              <EmptyDescription>
                ...
                {corpora.length} {corpora.length === 1 ? "corpus" : "corpora"}.
              </EmptyDescription>
            </EmptyHeader>
            <Button
              variant="outline"
        ...
        <ul className="flex flex-col gap-2" aria-label="Published corpora">
          {filtered.map((corpus) => (
            <li key={corpus.filename}>
              <button
                type="button"
                onClick={() => onSelect(corpus.filename)}
                className={cn(
                  "flex w-full cursor-pointer items-center gap-3 rounded-2xl border-2 border-border bg-background p-3 text-left transition-colors",
                  "hover:border-amber-400 hover:bg-muted/50 dark:hover:border-amber-500",
